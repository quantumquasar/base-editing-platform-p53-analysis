...
x <- data.frame(rtracklayer::import("../genome/gencode.v46.primary_assembly.annotation.gtf"))
...
#61510
length(unique(rownames(count_matrix)))
#63140
...
#some rownames are duplicated (gene names are the same, but versions: ****.1, ****.2)
...
vsd <- vst(dds, blind = F)
#prefer this, since n of samples = 54
meanSdPlot(assay(vsd), ranks = F)

#normalized counts
dds_norm <- counts(dds, normalized = T)
meanSdPlot(log2(dds_norm + 1), ranks = F)
#first two samples
meanSdPlot(log2(dds_norm[,1:2] + 1), ranks=F)
...
EnhancedVolcano(res_PANC1_WT_vs_36,
                lab = rownames(res_PANC1_WT_vs_36),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'PANC1 WT vs 36',
                ylab = bquote(`p-value`),
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_PANC1_WT_vs_48,
                lab = rownames(res_PANC1_WT_vs_48),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, PANC1 48 vs WT',
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_PANC1_WT_vs_72,
                lab = rownames(res_PANC1_WT_vs_72),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, PANC1 72 vs WT',
                pCutoff = 0.05,
                FCcutoff = 1)
...
ggsave("plots/volcanoes_overlapping_116_targets/PANC1_@_72_labsize_10.svg", plot = p, 
...
EnhancedVolcano(res_A431_36,
                lab = rownames(res_A431_36),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, A431 at 36h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_A431_48,
                lab = rownames(res_A431_48),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, A431 at 48h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_A431_72,
                lab = rownames(res_A431_72),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, A431 at 72h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
ggsave("plots/volcanoes_overlapping_116_targets/A431_@_72_labsize_10.svg", plot = p, 
       width = 14, height = 10, dpi = 300, device = grDevices::svg)
...
EnhancedVolcano(res_HT29_36,
                lab = rownames(res_HT29_36),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, HT29 at 36h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_HT29_48,
                lab = rownames(res_HT29_48),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, HT29 at 48h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
EnhancedVolcano(res_HT29_72,
                lab = rownames(res_HT29_72),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'padj<0.05, lfc>1, HT29 at 72h',
                pCutoff = 0.05,
                FCcutoff = 1)
...
ggsave("plots/volcanoes_overlapping_116_targets/HT29_@_72_labsize_10.jpeg", plot = p, 
       width = 14, height = 10, dpi = 300, device = grDevices::jpeg, units = "in")
...
p <- EnhancedVolcano(res_H311_36,
                     ...
                     labSize = 4,
                     labFace = 'italic',
                     title = 'padj<0.05, lfc>2, SMAD4 at 36h',
...
p <- EnhancedVolcano(res_H311_48,
                     ...
                     labSize = 4,
                     labFace = 'italic',
                     title = 'padj<0.05, lfc>2, SMAD4 at 48h',
